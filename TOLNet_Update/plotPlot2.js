...
$(function(){
	...
	var Plot = ( function(window, d3, filename){
			...
			if(type){
				...
				cards = csvg.selectAll(".profile")
					.data(data, function(d){
						var AltmaxMin = d3.extent(d.PROFILE, function(n){return n.ALTITUDE;});

						return AltmaxMin[0] + ':' + d["DATETIME.START"]; 
					});


				cards.enter().append("rect")
					.attr("x", function(d){ return cx(str2jsDate(d["DATETIME.START"])) + cmargin.left + 1; })
					.attr("y", function(d){ return cy(function(n){
														Altmaxmin = d3.extent(n.PROFILE, function(z){return z.ALTITUDE;}); 
														return Altmaxmin[1];
													}(d)) + cmargin.top - 3; })
					...
					.attr("height",function(d){ 
						var Altmaxmin = d3.extent(d.PROFILE, function(n){return n.ALTITUDE;});
						return  cy(Altmaxmin[0])-cy(Altmaxmin[1]);
					})
					.attr("fill", function(d){ 
						var Altmaxmin = d3.extent(d.PROFILE, function(n){return n.ALTITUDE;});
						var thisProfHeight = (cy(Altmaxmin[0])-cy(Altmaxmin[1]));
						var datapointSpacing= thisProfHeight/(d.PROFILE.length - 1);

						/*var pdomain = function(n){
							var alts =[];
							n.forEach(function(g,i){
								alts.push(g.ALTITUDE);
							});
							return alts;
						};*/
						
						var prange = [];
						d.PROFILE.forEach(function(g,i){
							prange.push(colorScale(g["O3.MIXING.RATIO.VOLUME_DERIVED"]));
						});
							
						/*var profileColors = d3.scale.threshold()
							.domain(pdomain(d.PROFILE))
							.range(prange(d.PROFILE));*/

						// var profile = csvg.select(".profile")
						// 	.data(profileColors.domain(), function(n){return n;})
						// 	.enter().append("g")
						// 	.attr("class","profile");

						var id = "profile" + d["DATETIME.START"];
						var profile = csvg.append("defs")
						...
						for(var i = 0; i < d.PROFILE.length; i++){
							offset = i * datapointSpacing;
							profile.append('stop') 
								.attr('offset',offset/thisProfHeight)
								.attr('stop-color', prange[i] );
							profile.append('stop') 
								.attr('offset',offset/thisProfHeight)
								.attr('stop-color', prange[i] );
						}
						...
						return "url(#profile" + d["DATETIME.START"] + ")"; 
					})
	            ...
	            ppath = pplotWrapper.append('path').datum(data[0].PROFILE).classed('line',true);
				ppath.attr('d', pline);
		...
		function initProfile(div){
			//initialize scales
			pxExtent = d3.extent(data[0].PROFILE, function(d,i){return d["O3.MIXING.RATIO.VOLUME_DERIVED"]});
			pyExtent = d3.extent(data[0].PROFILE, function(d,i){return d.ALTITUDE});
			...
			cxExtent = d3.extent(function(darray, names){
				...
				darray.forEach(function(d){
					names.forEach(function(n){
						values.push(str2jsDate(d[n]));
					});
				});
				return values;
			}(data, ["DATETIME.START","DATETIME.STOP"]));
			cyExtent = d3.extent(function(darray){
				var values = [];
				darray.forEach(function(d){
					d.PROFILE.forEach(function(n){
						values.push(n.ALTITUDE);
					});
				});
				return values;
			}(data));
	...
	function populatePage(){
		...
		datasetpicker.enter()
			...
			.on("click", function(d){
				...
				while(maindiv.firstChild){  // REMOVES OLD SVG ELEMENT
